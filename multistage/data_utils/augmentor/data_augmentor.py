...
from multistage.data_utils.augmentor import augment_utils_v2v4real
...
class DataAugmentor_v2v4real(object):
    """
    Data Augmentor.
    ...
    def random_world_flip(self, data_dict=None, config=None):
        ...
        for i, cur_axis in enumerate(config['ALONG_AXIS_LIST']):
            assert cur_axis in ['x', 'y']
            gt_boxes_valid, points = getattr(augment_utils_v2v4real,
                                             'random_flip_along_%s' % cur_axis)(
    ...
    def random_world_rotation(self, data_dict=None, config=None):
        ...
        gt_boxes_valid = gt_boxes[gt_mask == 1]
        gt_boxes_valid, points = augment_utils_v2v4real.global_rotation(
    ...
    def random_world_scaling(self, data_dict=None, config=None):
        ...
        gt_boxes_valid, points = augment_utils_v2v4real.global_scaling(
            gt_boxes_valid, points, config['WORLD_SCALE_RANGE'],
            noise_scale
        )
        gt_boxes[:gt_boxes_valid.shape[0], :] = gt_boxes_valid
