...
import multistage.utils.pcd_utils as pcd_utils
from multistage.data_utils.augmentor.data_augmentor import DataAugmentor
from multistage.hypes_yaml.yaml_utils import load_yaml
from multistage.utils.pcd_utils import downsample_lidar_minimum
from multistage.utils.transformation_utils import x1_to_x2, dist_two_pose, tfm_to_pose, pose_to_tfm
...
class BaseDataset(Dataset):
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            # data[cav_id]['params']['lidar_pose'] = data[cav_id]['params']['gps']      
        return data
    ...
    def add_loc_noise_v2v4real(self, pose, xyz_std, ryp_std):
        """
        Add localization noise to the pose.
        ...
        ryp_std = np.random.normal(0, ryp_std, 3)
        R_delta = np.array([
            [np.cos(ryp_std[1]), -np.sin(ryp_std[1]), 0],
            [np.sin(ryp_std[1]), np.cos(ryp_std[1]), 0],
            [0, 0, 1]
        ]) 
        R = pose[:3, :3] @ R_delta
        T = pose[:3, 3] + xyz_noise
        T_prime = np.eye(4)
        T_prime[:3, :3] = R
        T_prime[:3, 3] = T
        return T_prime
    ...
    def augment_v2v4real(self, lidar_np, object_bbx_center, object_bbx_mask,
...
class BaseDataset_v2v4real(Dataset):
    """
