...
from multistage.utils import common_utils
...
def visualize_single_sample_output_gt(pred_tensor,
    ...
    origin_lidar_intcolor = \
        color_encoding(np.squeeze(origin_lidar), mode='constant')
    # left -> right hand
    origin_lidar[:, :1] = -origin_lidar[:, :1]

    pred_tensor_2d = -pred_tensor[:,:4,:2].clone()
    gt_tensor_2d = -gt_tensor[:,:4,:2].clone()
    pred_noise_bbx_2d = -vis_data['selected_bbx'][0][:,:4,:2].clone()

    # pred_tensor_2d[:,:,0] = -pred_tensor_2d[:,:,0]
    # gt_tensor_2d[:,:,0] = -gt_tensor_2d[:,:,0]

    pred_tensor_2d = (common_utils.torch_tensor_to_numpy(pred_tensor_2d) + np.array([140.8,40])) / np.array([0.8,0.8])
    gt_tensor_2d = (common_utils.torch_tensor_to_numpy(gt_tensor_2d) + np.array([140.8,40])) / np.array([0.8,0.8])
    pred_noise_bbx_2d = (common_utils.torch_tensor_to_numpy(pred_noise_bbx_2d) + np.array([140.8,40])) / np.array([0.8,0.8])

    color2 = [255.0, 153, 153]
    color1 = [255, 230, 100]
    color3 = [255,255,255]

    grid_y, grid_x = 100, 252

    mask_feature = common_utils.torch_tensor_to_numpy(vis_data['mask_feature'][0][0][0])
    mask_bbx = common_utils.torch_tensor_to_numpy(vis_data['mask_bbx'][0][0][0])   

    image = np.full((grid_y, grid_x, 3), color3, dtype=np.uint8)

    image[mask_feature == 1] = color1
    image[(mask_feature == 0) & (mask_bbx == 1)] = color2

    fig, ax = plt.subplots(figsize=(14, 4))
    ax.set_xlim(0, grid_x)
    ax.set_ylim(grid_y, 0)

    ax.imshow(image, extent=[0,grid_x, grid_y, 0])

    for bbox in gt_tensor_2d:
        rect2 = patches.Polygon(bbox, closed=True, edgecolor='green', linewidth=1.5, fill=False, alpha=0.6)
        ax.add_patch(rect2)

    for bbox in pred_noise_bbx_2d:
        rect3 = patches.Polygon(bbox, closed=True, edgecolor='blue', linewidth=1.5, fill=False, alpha=0.6)
        ax.add_patch(rect3)

    for bbox in pred_tensor_2d:
        rect1 = patches.Polygon(bbox, closed=True, edgecolor='red', linewidth=1.5, fill=False, alpha=0.6)
        ax.add_patch(rect1)
    ax.set_xticks([])
    ax.set_yticks([])

    # #plt.show()
    # plt.savefig(f"/home/tengjian/code/MultiStage_Augment/multi/tools/vis_result/output_{i}.png", dpi=300, bbox_inches='tight')
    ...
    gt_tensor[:,:,2]=0.01
    pred_tensor[:,:,2]=0.01
    oabbs_gt = bbx2linset(gt_tensor[...,:]+torch.tensor([0,0,0], device=gt_tensor.device), color=(0, 1, 0))
    oabbs_pred = bbx2linset(pred_tensor[...,:]+torch.tensor([0,0,0.5], device=gt_tensor.device), color=(1, 0, 0))

    # Duplicate LineSet to simulate thicker lines
    oabbs_gt_thick = [duplicate_lineset(obj, offset=0.03) for obj in oabbs_gt]
    oabbs_pred_thick = [duplicate_lineset(obj, offset=0.03) for obj in oabbs_pred]

    visualize_elements = [o3d_pcd] + sum(oabbs_gt_thick, []) + sum(oabbs_pred_thick, [])
    if show_vis:
        custom_draw_geometry(o3d_pcd, sum(oabbs_gt_thick, []), sum(oabbs_pred_thick, []))
    if save_path:
        save_o3d_visualization(visualize_elements, save_path)


def duplicate_lineset(lineset, offset=0.02):
    lineset_list = []
    for i in range(8):
        new_lineset_pos = o3d.geometry.LineSet()
        new_lineset_neg = o3d.geometry.LineSet()
        
        new_lineset_pos.points = o3d.utility.Vector3dVector(np.asarray(lineset.points) + (i * offset))
        new_lineset_pos.lines = o3d.utility.Vector2iVector(np.asarray(lineset.lines))
        new_lineset_pos.colors = o3d.utility.Vector3dVector(np.asarray(lineset.colors))
        
        new_lineset_neg.points = o3d.utility.Vector3dVector(np.asarray(lineset.points) - (i * offset))
        new_lineset_neg.lines = o3d.utility.Vector2iVector(np.asarray(lineset.lines))
        new_lineset_neg.colors = o3d.utility.Vector3dVector(np.asarray(lineset.colors))
        
        lineset_list.append(new_lineset_pos)
        lineset_list.append(new_lineset_neg)
    
    return lineset_list

def visualize_single_sample_output_bev(pred_box, gt_box, pcd, vis_data, 
                                       show_vis=True,
                                       save_path=''):
    """
