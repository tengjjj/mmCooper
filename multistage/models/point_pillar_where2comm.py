...
from multistage.models.sub_modules.naive_compress import NaiveCompressor
# from opencood.models.fuse_modules.where2comm import Where2comm
from multistage.models.fuse_modules.where2comm_attn import Where2comm
import torch
...
class PointPillarWhere2comm(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.multi_scale:
            fused_feature, comm_feature, result_dict = self.fusion_net(batch_dict['spatial_features'],
                                            psm_single,
                                            record_len,
                                            pairwise_t_matrix, 
                                            self.backbone,
            ...
            fused_feature, comm_feature, result_dict = self.fusion_net(spatial_features_2d,
                                            psm_single,
                                            record_len,
                                            pairwise_t_matrix)
            
        total_comm = torch.log2((comm_feature * 64) * 32 / 8)
        ...
        rm_single_v = []
        rm_single_i = []
        for b in range(len(split_rm_single)):
            # psm_single_v.append(torch.ones(1,2,100,252).to(device=psm_single.device))
            # psm_single_i.append(torch.ones(1,2,100,252).to(device=psm_single.device))
