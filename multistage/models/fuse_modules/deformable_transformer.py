...
class RPN_transformer_deformable_mtf_singlescale(nn.Module):
    def __init__(self,channel, args):
        ...
        self.pos_embedding = nn.Linear(140, self.channels)
        self.position_embedding = nn.Linear(40,self.channels)
        self.scale = [1,0.5,0.25]
        self.lidar_range = args['lidar_range']

    def forward(self, x, bbx_feature, selected_bbx, left_hand, selected_score):
        voxel_size = 0.4
        ...
        bbx_coordinate = selected_bbx[:,:2].clone()  # obj_nums,2
        if left_hand: 
        ...
        unique_indices = self.del_same_bbx(coords, selected_score)

        unique_coords = coords[unique_indices]
        bbx_feature = bbx_feature[unique_indices]
        selected_bbx = selected_bbx[unique_indices]

        order = (unique_coords[:,0] + unique_coords[:,1]*W).unsqueeze(0)  # 1,obj_nums
        ...
        # position = unique_coords.unsqueeze(0)
        pos_embed = self.position_encoding(position.squeeze(0))
        ...
        spatial_shapes = torch.as_tensor(
            spatial_list, dtype=torch.long, device=bbx_feature.device
        )
